...
import (
	...
	buildsapi "git.sr.ht/~sircmpwn/builds.sr.ht/api"
...
func (r *mutationResolver) Submit(ctx context.Context, manifest string, tags []string, note *string, secrets *bool, execute *bool, visibility *model.Visibility) (*model.Job, error) {
	...
	man, err := buildsapi.LoadManifest(manifest)
	if err != nil {
		return nil, err
	}
	...
	if man.OAuth != "" {
		want, err := auth.DecodeGrants(ctx, man.OAuth)
		if err != nil {
			return nil, err
		}

		// If using an OAuth token to authenticate with the API, ensure
		// that the grants required by the manifest are a subset of the
		// OAuth token grants (to prevent privilege escalation).
		if user.AuthMethod == auth.AUTH_OAUTH2 {
			if !want.IsSubset(&user.Grants) {
				return nil, fmt.Errorf("cannot submit build with requested OAuth permissions")
			}
		}
	}
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err := row.Scan(&job.ID, &job.Created, &job.Updated, &job.Manifest,
			...
			if err == sql.ErrNoRows {
				return fmt.Errorf("no pending jobs with this ID on your account")
			}
			return err
		}

		man, err := buildsapi.LoadManifest(job.Manifest)
		if err != nil {
...
func (r *mutationResolver) Cancel(ctx context.Context, jobID int) (*model.Job, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err := row.Scan(append(
			...
			if err == sql.ErrNoRows {
				return fmt.Errorf("no running jobs for your account with this ID")
			}
			return err
		}
...
func (r *mutationResolver) Update(ctx context.Context, jobID int, visibility model.Visibility, tags []string) (*model.Job, error) {
	var job model.Job
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if tags == nil {
			row = tx.QueryRowContext(ctx, `
				...
				id, created, updated, manifest, note, image, runner, owner_id,
				tags, status, visibility
				`, visibility, jobID, auth.ForContext(ctx).UserID)
		} else {
			row = tx.QueryRowContext(ctx, `
				UPDATE job
				SET visibility = $1, tags=$2
				WHERE id = $3 AND owner_id = $4
				RETURNING
				id, created, updated, manifest, note, image, runner, owner_id,
				tags, status, visibility
				`, visibility, strings.Join(tags, "/"), jobID, auth.ForContext(ctx).UserID)
		}
		return row.Scan(&job.ID, &job.Created, &job.Updated, &job.Manifest,
			&job.Note, &job.Image, &job.Runner, &job.OwnerID, &job.RawTags, &job.RawStatus,
			&job.Visibility)
	}); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
	...
	webhooks.DeliverUserJobEvent(ctx, model.WebhookEventJobUpdated, &job)
	return &job, nil
}
...
func (r *mutationResolver) StartGroup(ctx context.Context, groupID int) (*model.JobGroup, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err := row.Scan(&group.ID, &group.Created,
			&group.Note, &group.OwnerID); err != nil {
			if err == sql.ErrNoRows {
				return fmt.Errorf("no job group by this ID for your account")
			}
			return err
		}
...
func (r *mutationResolver) ShareSecret(ctx context.Context, uuid string, user string) (model.Secret, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		row := tx.QueryRowContext(ctx, `
			...
			ON CONFLICT ON CONSTRAINT secret_user_id_uuid_unique
...
func (r *mutationResolver) CreateUserWebhook(ctx context.Context, config model.UserWebhookInput) (model.WebhookSubscription, error) {
	...
	for i, ev := range config.Events {
		...
		var access string
		switch ev {
		case model.WebhookEventJobCreated, model.WebhookEventJobUpdated:
			access = "JOBS"
		default:
...
func (r *queryResolver) Version(ctx context.Context) (*model.Version, error) {
	...
	sshUser, ok := conf.Get("builds.sr.ht", "ssh-user")
	if !ok {
		sshUser = "builds"
	}
...
func (r *queryResolver) Jobs(ctx context.Context, cursor *coremodel.Cursor) (*model.JobCursor, error) {
	...
	return &model.JobCursor{Results: jobs, Cursor: cursor}, nil
}
...
func (r *queryResolver) Secrets(ctx context.Context, cursor *coremodel.Cursor) (*model.SecretCursor, error) {
	...
	return &model.SecretCursor{Results: secrets, Cursor: cursor}, nil
}
