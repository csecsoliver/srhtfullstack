...
func (r *labelResolver) Tickets(ctx context.Context, obj *model.Label, cursor *coremodel.Cursor) (*model.TicketCursor, error) {
	...
	return &model.TicketCursor{Results: tickets, Cursor: cursor}, nil
}
...
func (r *mutationResolver) CreateTracker(ctx context.Context, name string, description *string, visibility model.Visibility, importUpload *graphql.Upload) (*model.Tracker, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		row := tx.QueryRowContext(ctx, `
			...
			RETURNING id, rid, owner_id, created, updated, name, description,
				visibility, default_access;
		`, user.UserID, name, description, visibility.String(), importReader != nil)

		if err := row.Scan(&tracker.ID, &tracker.RID, &tracker.OwnerID, &tracker.Created,
	...
	log.Printf("tracker create: %s created tracker '%s'",
		user.Username, name)
...
func (r *mutationResolver) UpdateTracker(ctx context.Context, id int, input map[string]interface{}) (*model.Tracker, error) {
	...
	validation.NullableString("description", func(desc *string) {
		if desc != nil {
	...
	validation.Optional("visibility", func(vis any) {
		query = query.Set(`visibility`, vis)
	})

	newName := ""
	validation.OptionalString("name", func(name string) {
		validation.Expect(trackerNameRE.MatchString(name),
			"Invalid tracker name '%s' (must match %s)",
			name, trackerNameRE.String()).
			WithField("name")
		validation.Expect(name != "." && name != "..",
			"Invalid tracker name '%s' (must not be . or ..)", name).
			WithField("name")
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if newName != "" {
			_, err := tx.ExecContext(ctx, `
				INSERT INTO redirect (
					created, name, owner_id, new_tracker_id
				) SELECT
					NOW() at time zone 'utc',
					orig.name, orig.owner_id, orig.id
				FROM tracker orig
		...
		if err := row.Scan(&tracker.ID, &tracker.RID, &tracker.Created, &tracker.Updated,
...
func (r *mutationResolver) DeleteTracker(ctx context.Context, id int) (*model.Tracker, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		row := tx.QueryRowContext(ctx, `
			...
			WHERE id = $1 AND owner_id = $2
			RETURNING
				id, rid, owner_id, created, updated, name, description, visibility,
				default_access;
		`, id, user.UserID)
...
func (r *mutationResolver) UpdateTrackerACL(ctx context.Context, trackerID int, input model.ACLInput) (*model.DefaultACL, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		return row.Scan(&tracker.ID, &tracker.Created, &tracker.Updated,
			&tracker.Name, &tracker.Description, &tracker.Visibility,
			&tracker.DefaultAccess, &tracker.OwnerID)
	}); err != nil {
		if err == sql.ErrNoRows {
			return nil, gerrors.ErrNotFound
		}
...
func (r *mutationResolver) TicketUnsubscribe(ctx context.Context, trackerID int, ticketID int) (*model.TicketSubscription, error) {
	...
	if err != nil {
		...
		return nil, gerrors.New(gerrors.NotFound, "Ticket not found")
	}
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err == sql.ErrNoRows {
			return nil, gerrors.New(gerrors.NotFound, "Subscription not found")
		}
		return nil, err
	}
	return &sub, nil
...
func (r *mutationResolver) SubmitTicket(ctx context.Context, trackerID int, input model.SubmitTicketInput) (*model.Ticket, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err := row.Scan(&ticket.PKID, &ticket.ID, &ticket.SubmitterID,
			...
			&ticket.Body, &ticket.Authenticity, &ticket.Status,
			&ticket.Resolution); err != nil {
			return err
		}
	...
	log.Printf("ticket create: %s created ticket '%s'",
		user.Username, input.Subject)
...
func (r *mutationResolver) DeleteTicket(ctx context.Context, trackerID int, ticketID int) (*model.Ticket, error) {
	...
	if err != nil {
		...
		return nil, gerrors.New(gerrors.NotFound, "Tracker not found")
	}
	if !tracker.CanEdit() {
		return nil, gerrors.ErrAccessDenied
	}
...
func (r *mutationResolver) SubmitCommentEmail(ctx context.Context, trackerID int, ticketID int, input model.SubmitCommentEmailInput) (*model.Event, error) {
	...
	var (
		...
		_oldStatus     model.TicketStatus
		newStatus      *model.TicketStatus
		_newStatus     model.TicketStatus
		oldResolution  *model.TicketResolution
		_oldResolution model.TicketResolution
		newResolution  *model.TicketResolution
		_newResolution model.TicketResolution
	...
	if input.Cmd != nil {
		...
		case model.EmailCmdResolve:
			if input.Resolution == nil {
				return nil, fmt.Errorf("resolution is required when cmd is RESOLVE")
			}
			...
			*oldResolution = ticket.Resolution
			*newStatus = model.TicketStatusResolved
			*newResolution = *input.Resolution
		...
		case model.EmailCmdReopen:
			...
			*oldStatus = ticket.Status
			*oldResolution = ticket.Resolution
			*newStatus = model.TicketStatusReported
			*newResolution = model.TicketResolutionUnresolved
			updateTicket = updateTicket.
				Set("status", *newStatus).
				Set("resolution", *newResolution)
		}
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if details.StatusUpdated {
			details.Status = newStatus.String()
			details.Resolution = newResolution.String()
		}
...
func (r *mutationResolver) UpdateTicketStatus(ctx context.Context, trackerID int, ticketID int, input model.UpdateStatusInput) (*model.Event, error) {
	...
	if input.Status == model.TicketStatusResolved {
		if input.Resolution == nil {
			return nil, fmt.Errorf("resolution is required when setting status to RESOLVED")
		}
		resolution = *input.Resolution
		update = update.Set("resolution", resolution)
	} else {
		if input.Resolution != nil {
			return nil, fmt.Errorf("resolution may only be provided when status is set to RESOLVED")
		}
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		row := update.
			...
			RunWith(tx).
			QueryRowContext(ctx)
		if err := row.Scan(&ticket.Status, &ticket.Resolution); err != nil {
			return err
		}
...
func (r *mutationResolver) SubmitComment(ctx context.Context, trackerID int, ticketID int, input model.SubmitCommentInput) (*model.Event, error) {
	...
	validation := valid.New(ctx)
	validation.Expect(input.Import == nil ||
		(input.Import.ExternalID == "") == (input.Import.ExternalURL == ""),
		"Must specify both externalId and externalUrl, or neither, but not one")
	if !validation.Ok() {
		return nil, nil
	}
	...
	if input.Import != nil {
		validation.Expect(tracker.OwnerID == user.UserID,
			"Cannot configure for import unless you are the owner of this tracker")
		validation.Expect(strings.ContainsRune(input.Import.ExternalID, ':'),
			"Format of externalId field is '<third-party>:<name>', .e.g 'example.org:jdoe'").
			WithField("externalId")
		u, err := url.Parse(input.Import.ExternalURL)
		if err != nil {
			return nil, valid.Error(ctx, "externalUrl", err.Error())
		}
		if u.Scheme != "http" && u.Scheme != "https" {
		...
		var zeroDate time.Time
		validation.Expect(input.Import.Created != zeroDate,
			"Cannot use zero value for creation time").
			WithField("created")
	}
	if !validation.Ok() {
	...
	if input.Status != nil {
		...
		*newResolution = ticket.Resolution
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if input.Import != nil {
			...
			if err := row.Scan(&part.ID); err != nil {
				return err
			}
		} else {
			var err error
...
func (r *mutationResolver) AssignUser(ctx context.Context, trackerID int, ticketID int, userID int) (*model.Event, error) {
	...
	if err != nil {
		...
		return nil, gerrors.New(gerrors.NotFound, "User not found")
	}
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err, ok := err.(*pq.Error); ok &&
			...
			return fmt.Errorf("this user is already assigned to this ticket")
		} else if err != nil {
			return err
		}
...
func (r *mutationResolver) UnassignUser(ctx context.Context, trackerID int, ticketID int, userID int) (*model.Event, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		err := tx.QueryRowContext(ctx, `
			...
			Scan(&taId)
		if err == sql.ErrNoRows {
			return fmt.Errorf("this user is not assigned to this ticket")
		} else if err != nil {
			return err
		}
...
func (r *mutationResolver) LabelTicket(ctx context.Context, trackerID int, ticketID int, labelID int) (*model.Event, error) {
	...
	if err != nil {
		...
		return nil, gerrors.New(gerrors.NotFound, "Label not found")
	}
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if err, ok := err.(*pq.Error); ok &&
			...
			return fmt.Errorf("this label is already assigned to this ticket")
		} else if err != nil {
			return err
		}
...
func (r *mutationResolver) ImportTrackerDump(ctx context.Context, trackerID int, dump graphql.Upload) (bool, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		if n != 1 {
			return gerrors.ErrAccessDenied
		}
		return nil
	}); err != nil {
		return false, err
...
func (r *mutationResolver) CreateUserWebhook(ctx context.Context, config model.UserWebhookInput) (model.WebhookSubscription, error) {
	...
	for i, ev := range config.Events {
		...
		if !user.Grants.Has(access, auth.RO) {
			return nil, gerrors.Errorf(gerrors.AccessDenied, "Insufficient access granted for webhook event %s", ev.String())
		}
	}
...
func (r *mutationResolver) CreateTrackerWebhook(ctx context.Context, trackerID int, config model.TrackerWebhookInput) (model.WebhookSubscription, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		return row.Scan(&sub.ID, &sub.URL,
			&sub.Query, pq.Array(&sub.Events), &sub.UserID, &sub.TrackerID)
	}); err != nil {
		return nil, err
	}
...
func (r *mutationResolver) CreateTicketWebhook(ctx context.Context, trackerID int, ticketID int, config model.TicketWebhookInput) (model.WebhookSubscription, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		return row.Scan(&sub.ID, &sub.URL,
			&sub.Query, pq.Array(&sub.Events), &sub.UserID, &sub.TrackerID, &sub.TicketID)
	}); err != nil {
		return nil, err
	}
...
func (r *mutationResolver) UpdatePreferences(ctx context.Context, preferences model.PreferencesInput) (*model.Preferences, error) {
	...
	if err := database.WithTx(ctx, nil, func(tx *sql.Tx) error {
		row := tx.QueryRowContext(ctx, `
			UPDATE "user" SET notify_self = $1
			WHERE id = $2
			RETURNING notify_self;
		`, preferences.NotifySelf, auth.ForContext(ctx).UserID)
		return row.Scan(&notifySelf)
	}); err != nil {
		return nil, err
	}
...
func (r *queryResolver) Tracker(ctx context.Context, rid coremodel.RID) (*model.Tracker, error) {
	...
	if err := database.WithTx(ctx, &sql.TxOptions{
		...
		row := database.
			...
			From(`"tracker" tr`).
			LeftJoin(`user_access ua ON ua.tracker_id = tr.id AND ua.user_id = ?`, user.UserID).
			Column(`COALESCE(
				ua.permissions,
			...
			Where(sq.And{
				sq.Expr(`tr.rid = ?`, rid),
				sq.Or{
					sq.Expr(`tr.owner_id = ?`, user.UserID),
...
func (r *queryResolver) Events(ctx context.Context, cursor *coremodel.Cursor) (*model.EventCursor, error) {
	...
	return &model.EventCursor{Results: events, Cursor: cursor}, nil
}
	...
	case model.WebhookEventTicketUpdate:
		...
		webhook.Payload = &model.TicketEvent{
			...
			Ticket: &model.Ticket{
				...
				SubmitterID:  -1,
				Authenticity: model.AuthenticityAuthentic,
				Status:       model.TicketStatusReported,
				Resolution:   model.TicketResolutionUnresolved,
			},
		}
	case model.WebhookEventEventCreated:
		oldStatus := model.TicketStatusReported
		newStatus := model.TicketStatusResolved
		oldResolution := model.TicketResolutionUnresolved
		newResolution := model.TicketResolutionFixed
		participantId := -1
...
func (r *trackerResolver) Label(ctx context.Context, obj *model.Tracker, name string) (*model.Label, error) {
	...
	if err := database.WithTx(ctx, &sql.TxOptions{
		...
		return row.Scan(database.Scan(ctx, label)...)
	}); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (r *trackerResolver) Labels(ctx context.Context, obj *model.Tracker, cursor *coremodel.Cursor) (*model.LabelCursor, error) {
	...
	return &model.LabelCursor{Results: labels, Cursor: cursor}, nil
}
...
func (r *trackerResolver) Export(ctx context.Context, obj *model.Tracker) (*model.URL, error) {
	origin := config.GetAPI(config.ForContext(ctx), "todo.sr.ht", true)
